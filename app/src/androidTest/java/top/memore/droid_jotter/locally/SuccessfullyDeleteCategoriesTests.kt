package top.memore.droid_jotter.locally

import android.content.Context
import androidx.test.core.app.ApplicationProvider
import org.junit.After
import org.junit.Assert
import org.junit.Before
import org.junit.Test
import org.junit.runner.RunWith
import org.junit.runners.Parameterized

@RunWith(Parameterized::class)
class SuccessfullyDeleteCategoriesTests(
    private val c1: Category,
    private val c2: Category,
    private val deleted: Int
) {
    private lateinit var dao: LiteAccessible
    private lateinit var db: AppDatabase

    @Before
    fun setupDb() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        db = AppDatabase.getTestDatabase(context)
        dao = db.getAccessor()
    }

    @After
    fun closeDb() {
        db.close()
    }

    @Test
    fun deleteExistingCategories_existentDeleted() {
        dao.upsertCategories(c1, c2)

        Assert.assertEquals(deleted, dao.deleteCategories(c1, c2))
        Assert.assertEquals(0, dao.getCategories().size)
    }

    @Test
    fun deleteCategories_withNonexistent_existentDeleted() {
        dao.upsertCategories(c1, c2)

        val nonexistent = Category(nId = 30L, title = "Three")
        Assert.assertEquals(deleted, dao.deleteCategories(c1, c2, nonexistent))
        Assert.assertEquals(0, dao.getCategories().size)
    }

    companion object {
        @JvmStatic
        @Parameterized.Parameters
        fun testData(): Collection<Array<Any>> {
            return listOf(
                arrayOf(
                    Category(nId = 10L, title = "One"),
                    Category(nId = 11L, title = "Two"),
                    2
                ),
                arrayOf(
                    Category(nId = 10L, title = "One", millitime = -10001L),
                    Category(nId = 11L, title = "Two", millitime = -10002L),
                    2
                )
            )
        }
    }

}